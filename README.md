...
最新の公開版は `0.41.1` です。PalmWiki HomeなどのカスタムビューからMarkdownを
初めて開く場合も、表示領域の準備を待って本文内2-hop表示を描画します。また、
長文ノートの右上ボタンで最上部へ戻る際に途中で止まる場合を検出して補正し、
利用者が手動でスクロールした場合はその操作を優先します。
...
The latest release is `0.41.1`. Inline 2-hop rendering now waits for the real
Markdown host when a note is first opened from a custom view such as PalmWiki
Home. The upper-right control also detects and corrects an interrupted return to
the note top, while still giving manual scrolling priority.
