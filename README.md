...
最新の公開版は `0.40.0` です。この版では、Obsidian のレイアウトとリンク解決が
落ち着いてから初回計算を始め、短時間に重なったタブ切替は最後の要求だけを処理します。
同じリンクグラフ、直近ノートの表示結果、Canvas のリンク索引を再利用するため、
関連度順や `Related, Cosense-like` を保ったまま再計算を減らします。
...
The latest release is `0.40.0`. It waits for the workspace layout and link
resolution before the first calculation, coalesces rapid tab events into the
latest request, and reuses the link graph, recent note results, and the Canvas
link index. Relevance and `Related, Cosense-like` sorting remain available while
repeated full-graph work is reduced.
