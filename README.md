...
- 2-hop 表示上部から直接切り替えられるソート順プルダウン
...
最新の公開版は `0.38.0` です。`main` ブランチでは、ツールバーからソート順を
切り替えられる `0.39.0` を開発しています。
...
- A sort-order dropdown directly in the 2-hop toolbar
...
The latest release is `0.38.0`. The `main` branch is developing `0.39.0`, which
adds direct sort-order switching to the toolbar.
