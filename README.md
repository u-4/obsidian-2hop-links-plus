...
- 長文ノートの上部と2-hop表示を往復できる、円形のヘッダーボタン
- カスタム画面からMarkdownノートを開く際の、画面準備待ちと古い描画の中止
...
最新の公開版は `0.41.0` です。長文Markdownノートの右上に、ノート最上部と
本文内2-hop表示を往復するボタンを追加しました。検索欄は利用可能な幅まで広がり、
設定ボタンと一時ソートは右側にまとまります。separate paneの一時ソートも
ペインや検索状態を保ったまま更新されます。
...
問題が起きた場合は、バックアップした3ファイルを元の場所へ戻し、プラグインを
再読み込みしてください。更新作業自体はVaultのノートや添付ファイルを変更しません。
...
The latest release is `0.41.0`. It adds an upper-right control for moving
between the top of a long Markdown note and its inline 2-hop results. The search
field uses the available width, settings and temporary sorting stay grouped on
the right, and temporary sorting in a separate pane now preserves that pane and
its search state.
