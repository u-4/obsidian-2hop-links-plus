...
最新の公開版は `0.38.0` です。`main` ブランチでは、設定画面で既定ソートを選び、
ツールバーでは現在の表示だけを一時的に並び替えられる `0.39.0` を開発しています。
...
keeps the default sort order in settings while allowing temporary sorting in
the current toolbar.
